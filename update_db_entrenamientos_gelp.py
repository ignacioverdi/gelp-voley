...
def _sumar_al_historial(entrenamientos, output_dir='.'):
    ...
    dashboard, el historial y el analisis leen datos_historial.js. Sin este
    paso ese archivo queda vacio y la app no muestra ninguna sesion.
    """
    import json
    import os as _os
    import re as _re
...
def generate_team_pages_data(dvw_dir, team_name, output_dir='.', temporada='2025/26'):
    ...
    # y el historial miran datos_historial.js
    ...
    pjs += 'const ENTRENAMIENTOS_TRANSICION = ' + json.dumps(_trans_acum, ensure_ascii=False) + ';\n'
    with open(os.path.join(output_dir,'datos_entrenamientos.js'),'w',encoding='utf-8') as f: f.write(pjs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--db_path',    default='entrenamientos_nafels_db.json', help='Database file path')
    ...
    print(f"\n{'='*60}")
    print(f"NAFELS ENTRENAMIENTOS UPDATE — {args.temporada}")
    print(f"{'='*60}\n")
    ...
    with open(os.path.join(args.output_dir,'entrenamientos_full_stats.json'),'w') as f:
    ...
    print("\n4. Building liga_data_entrenamientos.js (modo entrenamiento)...")
    ...
    print(f"   \u2713 liga_data_entrenamientos.js ({len(combos)} combos, {len(setters_map)} equipos con armadores)")

    # Step 5: Build stats table (protegido: si falta template, no tumba el proceso)
    print("\n5. Building stats table...")
    try:
        print("   (tabla de liga NLA: NO se regenera en modo entrenamiento)")
    except Exception as e:
        print(f"   (stats table omitida: {e})")

    # Step 6: datos_partidos.js (para el equipo principal) — CRÍTICO, mostrar errores
    print("\n6. Building datos_entrenamientos.js...")
    generate_team_pages_data(args.dvw_dir, MAIN_TEAM, args.output_dir, args.temporada)
    print("   \u2713 datos_entrenamientos.js")

    # Step 7: (DESACTIVADO) Antes generaba 32 paginas por club
    # (ataque_/saque_/recepcion_/armador_<equipo>.html, una por equipo x 4 tipos).
    # Ahora se usan 4 paginas universales (ataque.html / saque.html / recepcion.html /
    # armador.html) que leen TODO de liga_data.js via ?team=<slug>. No hace falta
    # generar una hoja por equipo: con liga_data.js (paso 4) alcanza.
    print("\n7. Paginas por club: DESACTIVADO (sistema universal lee de liga_data.js).")
    # Para reactivar el sistema viejo de 32 hojas, descomentar la linea siguiente:
    # build_heatmaps(teams_data, template_dir=args.template_dir, output_dir=args.output_dir, temporada_filter=None)

    print(f"\n{'='*60}")
    print(f"\u2713 LISTO — {len(games_log)} sesiones de entrenamiento en la base")
    print(f"  Subir a GitHub: liga_data_entrenamientos.js, datos_entrenamientos.js, datos_historial_ent.js, datos_recepcion_ent.js")
    print(f"{'='*60}\n")
