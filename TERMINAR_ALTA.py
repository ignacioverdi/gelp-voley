...
"""
import os
import json, re, sys, json, time, glob, argparse, subprocess
...
def main():
    ...
    # ══ El calendario sale de config_club.json ════════════════════════════
    # Antes se leia config_temporada.js, un archivo que los clubes nuevos NO
    # tienen. Sin el quedaba el valor por defecto —mes 8, calendario europeo—
    # y un torneo argentino que empieza y termina el mismo año se etiquetaba
    # como "2025/26". Los datos quedaban guardados con un nombre y la app los
    # buscaba con otro: todas las pantallas vacias, sin ningun aviso.
    #
    # config_club.json es el mismo archivo que usa el resto del sistema, asi
    # la etiqueta coincide siempre.
    inicio, cruza = 8, True
    try:
        cj = os.path.join(AQUI, 'config_club.json')
        if os.path.exists(cj):
            _d = json.load(open(cj, encoding='utf-8'))
            _t = (_d.get('torneos') or {})
            if _t:
                _uno = list(_t.values())[0]
                if isinstance(_uno, dict):
                    inicio = int(_uno.get('inicio', inicio))
                    cruza = bool(_uno.get('cruza', True))
        else:
    ...
    if cruza:
        ...
        # el torneo empieza y termina el mismo año: la etiqueta es ese año
        etiqueta = str(anio)
