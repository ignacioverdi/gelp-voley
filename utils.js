...
function effColor(v, meta){
  /* ══ UNA SOLA VERSION, Y ATADA AL SEMAFORO ════════════════════════════════
     No tiene logica propia: le pregunta a objClassify, la misma funcion que
     pinta las baterias. Asi el numero y la bateria no pueden mostrar colores
...
function objClassify(id,val){
  /* ══ EL SEMAFORO, MEDIDO CONTRA LA LIGA ═══════════════════════════════════
     Antes cada fundamento tenia cortes propios escritos a mano, sin relacion
     entre si: el mismo porcentaje daba distinto color segun el fundamento. Y
     eran fijos: si cambiabas el objetivo, el semaforo seguia con el viejo.

     Ahora el color dice DONDE ESTAS DENTRO DE LA LIGA, usando el rango real
     que ya guarda cada meta (min = el peor, obj = el mejor):
